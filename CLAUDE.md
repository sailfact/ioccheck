...
- **`scoring.rs`** — additive score capped at 100, then bucketed (`<20` low, `<50` medium, `<80` high, else critical). **Scoring matches on the `SourceFinding.source` string literal** (`"CISA KEV"`, `"URLhaus"`, `"ThreatFox"`, `"AlienVault OTX"`, etc.). These strings are the contract between a source module and scoring — renaming one in a source file without updating `scoring.rs` silently drops its contribution. `AbuseIPDB` and `AlienVault OTX` score by severity tier; everything else without an explicit arm (e.g. `NVD`) falls through the default severity-based arm.
